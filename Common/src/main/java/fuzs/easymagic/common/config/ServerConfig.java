package fuzs.easymagic.common.config;
...
public class ServerConfig implements ConfigCore {
    ...
    @Config(description = "Leftover vanilla enchanting tables will transform when trying to use them.")
    public boolean convertVanillaEnchantingTableWhenInteracting = true;
    @Config(category = REROLLING_CATEGORY, description = {
    ...
    @Config(category = REROLLING_CATEGORY,
            description = "Switch re-rolling cost from experience points to full enchantment levels, making re-rolls considerable more expensive.")
    ...
    @Config(category = ENCHANTING_POWER_CATEGORY,
            description = "Amount of bookshelves required to perform enchantments at the highest level.")
    @Config.IntRange(min = 0, max = 50)
    public int maxEnchantingPower = 15;
    @Config(category = ENCHANTING_POWER_CATEGORY,
            description = "Blocks without a full collision shape (e.g. torches & carpet) do not block bookshelves placed behind from counting towards current enchanting power.")
    ...
    @Config(category = ENCHANTING_POWER_CATEGORY, description = {
