package fuzs.easymagic.common.client.gui.screens.inventory;

import fuzs.easymagic.common.EasyMagic;
import fuzs.easymagic.common.client.gui.components.EnchantmentSlotButton;
import fuzs.easymagic.common.client.gui.components.RerollButton;
import fuzs.easymagic.common.client.util.EnchantmentTooltipHelper;
import fuzs.easymagic.common.config.ClientConfig;
import fuzs.easymagic.common.config.ServerConfig;
...
public class ModEnchantmentScreen extends EnchantmentScreen {
    ...
    /**
     * @see EnchantmentScreen#extractRenderState(GuiGraphicsExtractor, int, int, float)
     */
    @Override
    public boolean isHovering(int left, int top, int width, int height, double mouseX, double mouseY) {
        if (this.disableTooltip && left == 60 && top >= 14 && top <= 52 && width == 108 && height == 17) {
            return false;
        } else {
